...
class SocialSharing_Projects_Module extends SocialSharing_Core_BaseModule
{
    ...
    public function doFrontendStuff()
    {
        ...
        foreach ($projects as $project) {
			if(!empty($project->settings['where_to_show']) && in_array($project->settings['where_to_show'], array('membership'))) {
				// do nothing, because running "doShortcode" from Membership
			} else {
    ...
    public function doShortcode($attributes)
    {
		...
		if(!empty($attributes['membershipParams'])) {
			$project->settings['membershipParams'] = $attributes['membershipParams'];
		}
