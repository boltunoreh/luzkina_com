...
class SocialSharing_Projects_Builder_Network
{
    ...
    public function __construct(array $network)
    {
		...
		$this->mail_to_default = array_key_exists(
			'mail_to_default',
			$network
		) ? $network['mail_to_default'] : false;
    }
	 ...
	 * @return string
	 */
	public function getMailTextFormat()
	{
		return $this->mail_to_default;
	}
    ...
    public function getUrl(WP_Post $post = null, SocialSharing_Projects_Project $project = null)
    {
		...
		if($project && method_exists($project, 'isShowAtMembership') && $project->isShowAtMembership()) {
			$settings = $project->getSettings();
			if(!empty($settings['membershipParams']['url'])) {
				$pairs['{url}'] = urlencode($settings['membershipParams']['url']);
			}
			if(!empty($settings['membershipParams']['title'])) {
				$pairs['{title}'] = urlencode($settings['membershipParams']['title']);
			} else {
				$pairs['{title}'] = '';
			}
		}
